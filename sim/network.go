...
func (n *Network) Aggregate(sig []byte, actorID f3.ActorID, aggSignature []byte, signers *bitfield.BitField, actor2Index map[f3.ActorID]int) ([]byte, *bitfield.BitField) {
	// Fake implementation.
	...
	signers.Set(uint64(actor2Index[actorID]))

	return updatedAggSignature, signers
}

func (n *Network) VerifyAggregate(msg, aggSig []byte, signers *bitfield.BitField, actor2Index map[f3.ActorID]int) bool {
	...
	lastActorIndex := len(actor2Index)
	for {
		...
		verifiedSigners.Set(uint64(currentActorIndex))
	}
